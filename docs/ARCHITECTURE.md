...
- **Dev containers (`docker/Dockerfile` + `docker-compose.yml`):** persistent environments for continuous build/test. The host's `projects/` dir is bind-mounted at `/workspace` so the `../pigeon` path-deps resolve unchanged; cargo registry/target and the Gradle cache live in named volumes for caching across restarts. The image carries the full toolchain: Rust + `rustfmt`/`clippy` + Android targets + `cargo-ndk`, JDK 17, Android SDK (platform/build-tools 34) + NDK 26, and Gradle. **⚠ Stale as of 2026-07-04:** the `android/` Gradle build was upgraded to **AGP 9.2.1 / Gradle 9.6.1 / Kotlin 2.2.10 / compileSdk 36** (see ROADMAP "Toolchain upgrade"), which this image does *not* yet satisfy — it needs a JDK with a compiler (17+ is fine to *run* Gradle 9) and `platforms;android-36`. Bump the Dockerfile before relying on the containerised android lane. **Two services** because the NDK ships only x86_64 Linux binaries:
