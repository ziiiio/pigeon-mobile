...
**Next: M2 (sync + plaintext rooms)** — the `/sync` long-poll loop + state diffing in the core (`sync.rs`), the local store (`store.rs`; decide SQLite via `sqlx`/`rusqlite` and flag it — Tech Stack), a room list + timeline + plaintext send (`rooms.rs`), and the Compose room-list/timeline UI over them. Read the reference CLI's sync/room flows first. E2EE stays out of scope until M3. Mind the sync gotchas: opaque composite tokens (#5), long-poll cancellation on background (#6), coarse FFI batching (#7).
