...
**Next: M2.5 (plaintext send)** — core: `PigeonClient.send_message(room, body)` with **local echo** (a provisional event in the store) + a persisted send queue with retry (offline-first); reconcile the echo with the server's event on ack (server ignores txn ids — dedup locally). UI: a composer in the chat screen. Then M2.6 (invites & membership + the M2 e2e exit gate). E2EE stays out of scope until M3. **Wire contract confirmed** (server + reference CLI, M2.1 research): `/sync` returns only `rooms.join`; state/membership events flow inline in `timeline.events` (no separate `state` block); an invite doesn't surface until you're joined; the messages endpoint (`rooms/:id/messages`) takes only `limit` (no `from`/`dir` cursor yet); send txn ids are **not** honored server-side (dedup locally if needed).
