...
**Next: M4 (media, backup/restore, polish, push).** M4.1 media upload/download → M4.2 encrypted media → M4.3 encrypted device backup & restore (`pigeon-crypto` recovery-key backup) → M4.4 push (FCM — confirm the server's push contract first) → M4.5 polish. `pigeon-crypto` already exposes `create_backup`/`restore_from_backup` (recovery key + AEAD blob) for M4.3. **Wire contract confirmed** (server + reference CLI, M2.1 research): `/sync` returns only `rooms.join`; state/membership events flow inline in `timeline.events` (no separate `state` block); an invite doesn't surface until you're joined; the messages endpoint (`rooms/:id/messages`) takes only `limit` (no `from`/`dir` cursor yet); send txn ids are **not** honored server-side (dedup locally if needed).
