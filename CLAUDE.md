...
**Next: M1.3** — persist the token to the platform keystore (via a host callback; never the app DB in clear) and restore the session on launch (validate with `/account/whoami`). Then M1.4 (auth UI) / M1.5 (logout).

New deps across M1: `reqwest`, `serde_json`, `tokio`, the `uniffi` `tokio` feature, and `wiremock` (dev-only) — all in the sanctioned stack.
