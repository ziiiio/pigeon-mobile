...
- **M4.2 encrypted media** — the server stores opaque (client-encrypted) bytes on the same path (no special endpoint). BUT client-side file encryption needs a symmetric AEAD (random per-file key, key carried in the E2EE event) that `pigeon-crypto` does not expose — so it needs a **new crypto dependency decision** (against the "single crypto path" rule) or a `pigeon-crypto` addition. **Genuine fork — flag before building.**
...
**Next: M4.2 (encrypted media) — a genuine crypto-dep fork to flag with the user** (client-side file AEAD isn't in `pigeon-crypto`), then M4.5 (polish, scoped to feasible client-side items). M4.4 (push) stays blocked. `pigeon-crypto` already exposes `create_backup`/`restore_from_backup` (used by M4.3). **Wire contract confirmed** (server + reference CLI, M2.1 research): `/sync` returns only `rooms.join`; state/membership events flow inline in `timeline.events` (no separate `state` block); an invite doesn't surface until you're joined; the messages endpoint (`rooms/:id/messages`) takes only `limit` (no `from`/`dir` cursor yet); send txn ids are **not** honored server-side (dedup locally if needed).
