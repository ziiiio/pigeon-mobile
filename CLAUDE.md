...
**Next: M2.3 (room list + create/join)** — core: `list_rooms` (from the store), `create_room`, `join_room` FFI on `PigeonClient`; UI: a Compose room-list screen with live updates driven by `SyncObserver.on_change`. Then M2.4 (timeline + pagination), M2.5 (plaintext send with local echo + retry queue), M2.6 (invites & membership). E2EE stays out of scope until M3. **Wire contract confirmed** (server + reference CLI, M2.1 research): `/sync` returns only `rooms.join`; state/membership events flow inline in `timeline.events` (no separate `state` block); an invite doesn't surface until you're joined; the messages endpoint (`rooms/:id/messages`) takes only `limit` (no `from`/`dir` cursor yet); send txn ids are **not** honored server-side (dedup locally if needed).
