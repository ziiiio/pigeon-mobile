...
**Active phase: Phase M1 (identity: register/login/session).** M0 is complete bar a one-time visual emulator run (see below). **M1 progress:** M1.1 (the Client–Server HTTP client) has landed — `core/src/api.rs`: a reqwest+rustls `Api` with base-URL/token config, generic `get`/`post`/`put` helpers, and `P_`-error → typed `ApiError`/`ErrorCode` mapping (12 codes + `Other`), with host-side unit tests. Server discovery (`.well-known`) is deferred. **Next: M1.2** — `register`/`login` endpoint wrappers + FFI exposure (map `ApiError`→`CoreError`), following the reference CLI's call sequence exactly. New deps added in M1.1: `reqwest`, `serde_json` (both in the sanctioned stack).
