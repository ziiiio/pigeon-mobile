...
**Next: M1.4** — the Compose auth UI (pick homeserver, register, login, logout) driving the core's `register`/`login`/`restore_session`, with the Android `KeyStore` impl (EncryptedSharedPreferences / Keystore-wrapped) installed at startup. Then M1.5 (logout & token invalidation).
