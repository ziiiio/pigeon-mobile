...
**Next: M3.5 (encrypted send/receive)** — encrypt on send for encrypted rooms (route `send`/`flush_pending` through the engine → `p.room.encrypted` `{algorithm, ciphertext}`); decrypt inbound `p.room.encrypted` during sync and **cache the plaintext on first decrypt** (the ratchet advances and is persisted — Gotcha #3; store schema v3). Own outgoing messages can't be self-decrypted → the plaintext local echo dedups against the authoritative ciphertext event by event_id. Then M3.6 encrypted-room UX + the **M3 exit gate** (extend the `e2e/` lane: two clients exchange real MLS ciphertext; assert the server-stored event has no plaintext; negative: outsider/tampered fail cleanly). Do **not** reinvent crypto: wrap `pigeon-crypto` (`openmls`) only. Reference: `../pigeon/clients/cli/src/e2ee.rs` + `../pigeon/tests/integration/tests/e2ee_message.rs`. **Wire contract confirmed** (server + reference CLI, M2.1 research): `/sync` returns only `rooms.join`; state/membership events flow inline in `timeline.events` (no separate `state` block); an invite doesn't surface until you're joined; the messages endpoint (`rooms/:id/messages`) takes only `limit` (no `from`/`dir` cursor yet); send txn ids are **not** honored server-side (dedup locally if needed).
