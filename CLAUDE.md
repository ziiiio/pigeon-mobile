...
**Active phase: Phase M5 (iOS). M5.1 done (core packaged for Swift). M5.2 done (Hello-core SwiftUI smoke app builds + runs on an iOS simulator — the whole Rust → xcframework → UniFFI → Swift → SwiftUI pipeline proven on-device, backed by the macOS CI lane). M4 is complete to the extent the server allows — M4.1/M4.2/M4.3/M4.5 done; M4.4 (push) BLOCKED (no server contract). M3 complete (M3.1–M3.6, headline E2EE). M2 complete (M2.1–M2.6). M1 complete (M1.1–M1.5). Next: M5.3 (Apple OS integration).**
