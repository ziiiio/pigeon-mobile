...
**Next: M2.2 (the sync loop, `core/src/sync.rs`)** — long-poll `/sync` (`GET /_pigeon/client/v1/sync?since&timeout&limit`), persist the opaque `next_batch` verbatim, `store.apply_events(...)` the per-room `rooms.join[<id>].timeline.events`, expose a change stream over the FFI (observer callback), and honor cancellation on background/screen-close (Gotcha #6). Then M2.3 (room list + create/join), M2.4 (timeline + pagination), M2.5 (plaintext send), M2.6 (invites & membership). Read the reference CLI's sync/room flows first. E2EE stays out of scope until M3. **Wire contract confirmed** (server + reference CLI, M2.1 research): `/sync` returns only `rooms.join`; state/membership events flow inline in `timeline.events` (no separate `state` block); an invite doesn't surface until you're joined; the messages endpoint (`rooms/:id/messages`) takes only `limit` (no `from`/`dir` cursor yet); send txn ids are **not** honored server-side (dedup locally if needed).

New deps in M2 so far: `rusqlite` (`bundled`) — sanctioned. New deps across M1: `reqwest`, `serde_json`, `tokio`, the `uniffi` `tokio` feature; dev-only `wiremock` + `serial_test` — all in the sanctioned stack.
