...
**Next: M2.4 (timeline + pagination)** — core: a `TimelineEvent` FFI record + `PigeonClient.timeline(room_id, limit, before)` (store read; the store already back-paginates by depth) and backfill via `GET /rooms/:id/messages` (takes only `limit` — no cursor yet); UI: a chat screen rendering the timeline with scroll-to-load-older, opened by tapping a room. Then M2.5 (plaintext send: local echo + retry queue), M2.6 (invites & membership + the M2 e2e exit gate). E2EE stays out of scope until M3. **Wire contract confirmed** (server + reference CLI, M2.1 research): `/sync` returns only `rooms.join`; state/membership events flow inline in `timeline.events` (no separate `state` block); an invite doesn't surface until you're joined; the messages endpoint (`rooms/:id/messages`) takes only `limit` (no `from`/`dir` cursor yet); send txn ids are **not** honored server-side (dedup locally if needed).
