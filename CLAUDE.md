...
**Active phase: Phase M5 (iOS). M5.1 done (core packaged for Swift). M5.2 done (Hello-core SwiftUI smoke app builds + runs on an iOS simulator — the whole Rust → xcframework → UniFFI → Swift → SwiftUI pipeline proven on-device). M5.3 done (Apple OS integration — the real `Pigeon` app's KeychainKeyStore/OsLogSink/PhotoPicker/SyncController + `PigeonApp` callback install, verified by a real-Keychain unit suite + app-launch smoke on a simulator; APNs push BLOCKED, no server contract). All three iOS lanes are in macOS CI. M4 is complete to the extent the server allows — M4.1/M4.2/M4.3/M4.5 done; M4.4 (push) BLOCKED (no server contract). M3 complete (M3.1–M3.6, headline E2EE). M2 complete (M2.1–M2.6). M1 complete (M1.1–M1.5). Next: M5.4 (SwiftUI feature parity for the M1–M4 flows).**
