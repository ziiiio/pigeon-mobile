...
- **M3.5** (`core/src/e2ee.rs`/`store.rs`/`rooms.rs`/`sync.rs`/`api.rs`): transparent encrypted send/receive. **Send:** `flush_pending` encrypts for a room whose group we hold (`encrypt_for_send` → `p.room.encrypted {algorithm, ciphertext}` via generalised `api.send_event`); the sender sees the plaintext echo (can't self-decrypt), the authoritative ciphertext event dedups by `event_id`. **Receive:** `decrypt_pending` decrypts inbound `p.room.encrypted` in DAG order, exactly once, caching plaintext (store schema **v3**: `decrypted` + `decrypt_state`) — the ratchet is one-way and persisted (Gotcha #3). Run by the sync loop (after Welcome processing) + on `fetch_messages`. Pending-if-no-group-yet; terminal-fail otherwise; `TimelineEvent` renders plaintext / hidden / "⚠ Unable to decrypt". Negative tests: outsider, tampered, non-MLS bytes. mock-HTTP proves the wire carries ciphertext only. No FFI surface change.
