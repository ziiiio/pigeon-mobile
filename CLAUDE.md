...
**Active phase: Phase M5 (iOS) — COMPLETE to the extent the server allows. M5.1 done (core packaged for Swift). M5.2 done (Hello-core SwiftUI smoke app builds + runs on an iOS simulator — the whole Rust → xcframework → UniFFI → Swift → SwiftUI pipeline proven on-device). M5.3 done (Apple OS integration — the real `Pigeon` app's KeychainKeyStore/OsLogSink/PhotoPicker/SyncController + `PigeonApp` callback install, verified by a real-Keychain unit suite on a simulator; APNs push BLOCKED, no server contract). M5.4 done (SwiftUI feature parity for the M1–M4 flows — auth/session, room list + create/join incl. E2EE, timeline + send/receive, media, invite, backup/restore — driven entirely by the shared core with zero protocol/crypto in Swift; the FFI boundary held, no new core logic needed. Built + unit-tested (`run-tests.sh`, 12 tests) and launched on a simulator (`run-app.sh`) — the app boots into its SwiftUI UI without crashing). All four iOS scripts (`build-core.sh`/`run-hellocore.sh`/`run-tests.sh`/`run-app.sh`) are in the macOS CI lane. M4 is complete to the extent the server allows — M4.1/M4.2/M4.3/M4.5 done; M4.4 (push) BLOCKED (no server contract). M3 complete (M3.1–M3.6, headline E2EE). M2 complete (M2.1–M2.6). M1 complete (M1.1–M1.5). Next: M6 (hardening & release) — Android+iOS parity over one core is achieved.**

**⚠ Env correction (M5, 2026-07-06):** M5 was fully built, unit-tested, and run on a **macOS host** (Xcode 26.6, iOS 26.5 simulator runtime; Rust installed via rustup). The "macOS-gated / not runnable" caveats below were written from the Linux dev container and apply *only there* — on a Mac with Xcode, every M5 step runs. The one real block is **APNs push**, a *server* gap (the homeserver exposes no push/pusher/notification/FCM/APNs endpoint — re-verified against `../pigeon`'s client-v1 route table in M5.4), not a toolchain one.

**⚠ Environment note for M5:** this dev container is **Linux/amd64 with no Xcode/Swift toolchain**. iOS work splits into (a) platform-independent, verifiable here — Swift **binding generation** (`uniffi-bindgen … --language swift`, now in the Linux CI `core` lane) and the build scripting/CI/docs; and (b) **macOS-gated** — the `cargo build` for Apple targets (iOS SDK needed to link + C-compile `rusqlite`), `xcodebuild -create-xcframework`, and **all SwiftUI compilation** (`swift`/`xcodebuild` absent). M5.2–M5.4 (Hello-core SwiftUI, OS integration, parity screens) can only be compiled/validated on a Mac — do not write large unverifiable Swift blind in the container; scaffold + flag, mirroring how M4.4 push was handled. **M5.2 has since been built + run on a macOS host** (Xcode 26, `iPhone 16` simulator): `ios/build-core.sh` then `ios/run-hellocore.sh` — both are in the macOS CI lane. When continuing M5 on a Mac, use that recipe (build the xcframework, then build/run the app on a simulator) — the "macOS-gated" caveats do not apply to a Mac host. APNs push (part of M5.3) inherits M4.4's server block. **M0 is complete** — the Hello-core app was run on an arm64 emulator and renders Rust-computed values (`core_version()` + `self_test_crypto()`) with the M0.7 log sink reaching logcat; the full Rust→cargo-ndk→UniFFI→Kotlin→Compose pipeline round-trips on a real device. **M1 record:** M1.1–M1.5 have landed.
...
**M3 record so far:**
- **M3.1** (`core/src/e2ee.rs` + `session.rs`/`api.rs`): device identity & key publishing. **Content-addressed KeyPackage ids (2026-07-07, review finding P5):** `upload_keys` now derives each `key_id` from a hash of the package (`key_package_id`) instead of a fixed `kp-{index}`. The server dedups uploads on `(user, device, key_id)` with `ON CONFLICT DO NOTHING`, so the old scheme dropped a *republished* pool after an identity change (`restore_backup` mints a fresh identity) — peers then claimed KeyPackages of the discarded identity and their Welcome could never be joined. Hashing makes a same-bytes re-upload idempotent while new packages get fresh ids. Unit test `key_package_id_is_content_addressed`. The `E2ee` engine wraps `pigeon-crypto::Device` (the only crypto path), mirroring the reference CLI's engine but persisting MLS state through the host `KeyStore` (`pigeon.mls.state.v1`), not a file — `pigeon-crypto` has no pluggable storage, so it `export_storage()`s and persists `{pubkey, blob}` (base64) after every mutation (Gotcha #1: private keys under the keystore). `create`/`restore`/`clear`, `signature_public_key_b64`, `key_packages(n)`. register/login mint a fresh identity and best-effort publish it (`publish_device_keys` → `POST /keys/upload`: `device_keys` + 5 base64 KeyPackages); `restore_session` restores the engine (or mints+publishes); `logout` wipes MLS state too. `api.rs` gained `upload_keys`/`query_keys`/`claim_keys`/`send_to_device`; `CoreError: From<CryptoError>`. **Wire facts (verified against server `e2ee_message.rs` + the CLI):** all key material is opaque base64 the server never parses; identity is stamped server-side from the token (absent from bodies); group_id = room_id bytes; Welcomes ride to-device (`p.mls.welcome`), not the room DAG; encrypted messages are `p.room.encrypted` with `{algorithm:"p.mls.1", ciphertext:<b64>}`; `add_member` self-merges its own commit and returns the Welcome **and** the commit (see the C1 reconciliation below; member removal remains out of scope). New dep (flagged): `base64 = "0.22"` — transport encoding only (not crypto), matching the server workspace. Validated: 5 e2ee unit tests + updated M1 session tests; `cargo test`/`clippy`/`fmt` green. Internal engine — no FFI surface change.

- **M3.2** (`core/src/keys.rs`): key query/claim orchestration. `claim_all_devices(api, user_id)` — the query→per-device-claim sequencing the CLI's `invite` does (query the user's devices, claim one KeyPackage per device, skip devices out of packages). Returns `ClaimedKeyPackage { device_id, key_package_b64 }` for `add_member` (M3.4). The HTTP verbs themselves landed in M3.1. Validated: 2 mock-HTTP unit tests.

- **M3.3** (`core/src/e2ee.rs` + `sync.rs`): to-device + Welcome handling. The engine gained the group ops (`has_group`/`create_group`/`add_member`→Welcome/`join_from_welcome`; group id = room id bytes). The sync loop's `apply_to_device` folds `to_device.events`: each `p.mls.welcome` → `join_from_welcome`. **Idempotent (Gotcha #8):** the Welcome carries `room_id` out-of-band, so a re-delivered Welcome for a group we hold is skipped; bad Welcomes are logged+skipped, never fatal. Validated: engine group round-trip + garbage-Welcome negative + sync unit tests (join-then-dedup, skip non-Welcome/malformed).

- **M3.4** (`core/src/rooms.rs`): encrypted room creation + invite-with-Welcome. New FFI `create_encrypted_room(name, topic)` — `createRoom {encryption:true}` + `create_group(room_id)` (errors if E2EE unavailable). `invite` is now transparent: after the server invite, if we host the room's group it does the CLI's dance — `claim_all_devices` → `add_member` per device → `/sendToDevice p.mls.welcome` (`{welcome, room_id}`) per device (+ a `p.mls.commit` room-event broadcast per add, added in the C1 reconciliation below). Validated: mock-HTTP flag test + full invite-with-Welcome with a real second-device KeyPackage (asserts request shapes *and* replays the shipped Welcome to prove the invitee joins — real crypto); `assembleDebug` green (new binding).

- **M3.5** (`core/src/e2ee.rs`/`store.rs`/`rooms.rs`/`sync.rs`/`api.rs`): transparent encrypted send/receive. **Send:** `flush_pending` encrypts for a room whose group we hold (`encrypt_for_send` → `p.room.encrypted {algorithm, ciphertext}` via generalised `api.send_event`); the sender sees the plaintext echo (can't self-decrypt), the authoritative ciphertext event dedups by `event_id`. **Plaintext-downgrade guard (2026-07-07, review finding P1):** when we hold no group for a room, `flush_pending` now consults `store.is_room_encrypted` before the plaintext fallback — a message for an *encrypted* room with no group yet (e.g. joined but the Welcome hasn't been processed) is **held queued** (retried each sync; it goes out encrypted once the group is joined), never sent as `p.room.message`. Deciding on group-presence alone used to leak the body into an E2EE room (Gotcha #1). Store unit test `is_room_encrypted_reflects_the_marker` + mock-HTTP `send_into_an_encrypted_room_without_a_group_is_held_not_downgraded`. **Receive:** `decrypt_pending` decrypts inbound `p.room.encrypted` in DAG order, exactly once, caching plaintext (store schema **v3**: `decrypted` + `decrypt_state`) — the ratchet is one-way and persisted (Gotcha #3). Run by the sync loop (after Welcome processing) + on `fetch_messages`. Pending-if-no-group-yet; terminal-fail otherwise; `TimelineEvent` renders plaintext / hidden / "⚠ Unable to decrypt". Negative tests: outsider, tampered, non-MLS bytes. mock-HTTP proves the wire carries ciphertext only. No FFI surface change.

- **M3.6** (`android/.../rooms/`): encrypted-room UX. Create-dialog **encrypted toggle** (default on) → `createRoom(name, topic, encrypted)` picks `createEncryptedRoom` vs `createRoom`; the room list badges encrypted rooms and the chat top bar shows a 🔒; the "encryption enabled" line and "⚠ Unable to decrypt" fallback render from the core's `system_text` (no native parsing). Device-list visibility deferred (flagged — unneeded for small add-mostly groups). `assembleDebug` green.

**M3 exit gate ✅ (headline criterion met):** `e2e/two_clients_exchange_encrypted_messages` — Alice creates an encrypted room + invites Bob (claim → add_member → Welcome to-device), Bob's sync joins the group and **decrypts** Alice's message, the reply round-trips, and the room's stored events fetched **straight from the real server** contain only `p.room.encrypted` ciphertext (never plaintext). Outsider/tampered/non-MLS negatives are in the core's e2ee unit tests. New e2e dep: `reqwest` (test lane only, for the raw server assertion). Docker-gated (compiles in-container).

**Server-drift reconciliation (2026-07-16 — `../pigeon` hardened; its CORRECTNESS_REVIEW closed):**
- **C1 — MLS commit broadcast + apply (multi-member encrypted groups).** `pigeon-crypto::Device::add_member` now returns `AddOutcome{welcome, commit}` (was: the Welcome bytes) and adds `Device::process_commit`. Adding a *third* member advances the group epoch, so the earlier members must apply the broadcast commit or they fall a ratchet epoch behind and can no longer decrypt. The client reconciled the whole path: `e2ee.add_member` returns welcome+commit and gains `process_commit`; `rooms.welcome_to_group` ships the Welcome to the invitee **and broadcasts the commit as a `p.mls.commit` room event** (`send_event`) to existing members; the sync loop's decrypt pass became `process_inbound_mls` — one **DAG-ordered** pass over the store's `pending_mls` that applies `p.mls.commit` (via `process_commit`, skipping our own and non-applicable ones) and decrypts `p.room.encrypted`, interleaved so a commit lands before the ciphertext at its new epoch (Gotcha #3). `store` gained `pending_mls`/`set_commit_processed` (commits tracked exactly-once via the existing `decrypt_state` column) and hides `p.mls.commit` from the timeline. Wire fact (server + CLI): the **Welcome is to-device** (`p.mls.welcome`), the **commit is a room DAG event** (`p.mls.commit`, content `{commit}`), applied in timeline order. Tests: e2ee `third_member_commit_keeps_earlier_member_in_sync` (real 3-device crypto — the earlier member can't decrypt until it applies the commit; author/added-member skip; garbage commit fails cleanly); store `pending_mls_interleaves_commits_and_messages_in_dag_order`; rooms mock-HTTP `invite_into_encrypted_room_broadcasts_the_commit`. `cargo test`/`clippy`/`fmt` green.
- **H9/M9 — `P_LIMIT_EXCEEDED` (429) is now reachable.** The server's rate-limit hardening (`X-Forwarded-For` bypass fixed, per-key spray fixed) makes a 429 a normal runtime outcome, not a never-hit code. The client treated every non-`Network` error as fatal, so a 429 would kill the sync loop (→ signed out) and fail a queued send terminally (→ "not sent"). Added `ApiError::is_transient` (true for `Network` **and** `Server{LimitExceeded}`); `run_sync` now backs off + retries on any transient error (not just `Network`), and `flush_pending` leaves a rate-limited send queued instead of failing it. Tests: `is_transient_covers_network_and_rate_limit_only`; rooms mock-HTTP `rate_limited_send_stays_queued_then_recovers` (429 → still sending → recovers on next flush). `cargo test`/`clippy`/`fmt` green.
- **P6 — publish a reusable last-resort KeyPackage.** `pigeon-crypto` added `Device::new_last_resort_key_package()` and the server's `KeyPackage` wire wrapper gained `last_resort: bool` (default false; migration `0004`): once a device's one-time pool is claimed dry, `/keys/claim` serves the last-resort package repeatedly. The client uploaded only 5 one-time packages and no fallback, so after 5 claims a device became **un-addable** to new encrypted groups (invites shipped zero Welcomes). Now `E2ee.last_resort_key_package()` (persists, like `key_packages`) feeds `publish_device_keys`, which uploads it best-effort alongside the pool; `api.upload_keys` gained a `last_resort_b64: Option<&str>` param and flags every item explicitly (`last_resort: false` on one-time, `true` on the fallback, all with content-addressed P5 `key_id`s). The claim side already handled whatever package the server returns — publish-only change, no FFI surface change. Tests: e2ee `last_resort_key_package_is_usable_for_a_join` (add_member → Welcome → join → encrypt/decrypt over it — real crypto); api mock-HTTP `upload_keys_flags_the_last_resort_package`. `cargo test`/`clippy`/`fmt` green.
- **C4 — encrypted-media key/nonce hardening: verified, no client change.** `pigeon-crypto::decrypt_media` now requires the 28-byte key (16 AES ‖ 12 nonce) and rejects a key-nonce vs blob-nonce mismatch. The client already round-trips the whole `EncryptedMedia.key` opaquely through `ImageContent.key` (core-to-core; Gotcha #1), so the existing suite (`encrypted_image_round_trips_send_to_display` + the e2ee media negatives) passes unchanged against the hardened crate.

**M4 record so far:**
- **M4.2** (`../pigeon` `crates/crypto` + `core/src/e2ee.rs`/`rooms.rs` + `android/.../rooms/Chat*`): encrypted media. **Resolved the crypto-dep fork by adding `encrypt_media`/`decrypt_media` to `pigeon-crypto`** (AES-128-GCM via the existing openmls provider — no new dep; keeps ONE crypto path). Core `E2ee.encrypt_media`/`decrypt_media` wrap it. `send_image` takes raw bytes and is transparent: plaintext room → upload + `p.image` (M4.1 path); encrypted room → encrypt under a fresh per-file key, upload the *ciphertext*, and send the `p.image` content (URL + key) *inside* an E2EE'd `p.room.encrypted` message (key never uploaded — Gotcha #1). `download_image` downloads + decrypts in-core (key never leaves it — Cardinal Rule); `ImageContent.key` carries the per-file key for encrypted images. Tests: `pigeon-crypto` round-trip + negatives; core encrypted-image round-trip (uploaded bytes are ciphertext, wire carries no plaintext, a 2nd MLS device recovers message+media); mock-HTTP (no `p.image` leak) + render; `assembleDebug`.
- **M4.1** (`core/src/media.rs`/`api.rs`/`rooms.rs` + `android/.../rooms/Chat*`): media upload/download. `upload_media(bytes, content_type)` (raw `POST /media/v1/upload`; client-side 50 MiB guard → typed limit error, avoiding the server's bare 413) → `pigeon://` URI; `download_media(uri)`; `send_image(room, ImageContent, caption)` → `p.image` message. `TimelineEvent.image: Option<ImageContent>`. **Plaintext rooms only** — `send_image` refuses encrypted rooms (bytes not client-encrypted yet → would leak; that's M4.2). UI: composer **Attach** (system `PickVisualMedia` picker, plaintext rooms), inline thumbnails via `BitmapFactory` (no image-loading dep) + tap-to-view. Tests: URI parse + mock-HTTP (raw upload/download, `p.image` shape, oversize + encrypted refusal) + render; `assembleDebug` green.
- **M4.3** (`core/src/e2ee.rs`/`session.rs`/`api.rs` + `android/.../rooms/`): encrypted device backup & restore. `E2ee.create_backup()`/`restore_from_backup(recovery_key, blob)` wrap `pigeon-crypto` (no new crypto). `PigeonClient.backup()` stores the opaque blob via `PUT /room_keys/key/!e2ee-backup/mls-device-state` and returns the recovery key (user-saved secret, never sent to the server — Gotcha #1); `restore_backup(key)` fetches (`GET`, `None` on 404), restores in place, re-publishes keys. `api.rs` `put_room_key`/`get_room_key`. UI: overflow menu → Back up (shows the key once) / Restore (key input). Tests: engine round-trip + wrong-key/garbage negatives; mock-HTTP wire shapes; `assembleDebug`. Forward-secrecy caveat documented.

**M4 status / blockers (researched against the server + reference CLI):**
- **M4.1 media** — server-implemented: `POST /_pigeon/media/v1/upload` (raw bytes, `Content-Type` preserved) → `{content_uri:"pigeon://srv/id"}`, 50 MiB cap → **413** (raw, no `P_` envelope); `GET /media/v1/download/{srv}/{id}` → raw bytes; `GET /media/v1/thumbnail/{srv}/{id}?width&height` → PNG. **No CLI precedent for image messages** — define the content shape (`{msgtype:"p.image", body, url|file, info}`). Unblocked.
- **M4.2 encrypted media** — ✅ **done** (see M4 record). Resolved by adding `encrypt_media`/`decrypt_media` to `pigeon-crypto` (AES-128-GCM via the existing openmls provider, no new dep) — keeps the single crypto path. The server stores the client-encrypted bytes opaquely on the same media path.
- **M4.4 push** — **BLOCKED.** The homeserver exposes **no** pusher/FCM/APNs/notification endpoint (the server roadmap lists push as unbuilt future work). Real-time delivery is the `/sync` long-poll only. Nothing to integrate against — do not half-build; revisit when the server grows a push contract.
- **M4.5 polish** — ✅ **done, scoped:** message timestamps (from `origin_server_ts`, display-only), image a11y `contentDescription`s, existing theming. Read receipts/typing/profile lack server endpoints → documented blocked, not built (same gap as push).

**M4 status: complete to the extent the server allows.** M4.1 (media), M4.2 (encrypted media), M4.3 (backup/restore), M4.5 (feasible polish) are all done. **M4.4 (push) is genuinely BLOCKED** — the homeserver exposes no push/pusher/notification endpoint at all (verified against the full client-api router; the server's own roadmap lists push as unbuilt). Not deferred — there is nothing to build against. Revisit when the server grows a push contract.

**M5 record so far:**
- **M5.1** (`ios/`): the core packaged for Swift. Verified the whole FFI surface **generates to Swift** cleanly (`uniffi-bindgen --language swift` — added to the Linux `core` CI lane as a smoke check). `ios/build-core.sh` (macOS) cross-compiles the Apple targets + generates bindings + assembles the `xcframework`; `ios/PigeonCore/Package.swift` exposes it as a SwiftPM dep (`import PigeonCore`). Added a **macOS `ios` CI lane**. The xcframework assembly + Apple-target `cargo build` are macOS-gated (documented in `ios/README.md`), so only the bindings-generation half is validated in-container.
- **M5.2** (`ios/HelloCore/`): the Hello-core SwiftUI smoke app **source is scaffolded** — `HelloCoreApp.swift` (installs an `os_log` `LogSink`) + `ContentView.swift` (renders `coreVersion()` + `selfTestCrypto()`), every call matched to the verified generated Swift signatures. **It can't be compiled/run in the Linux container (no Xcode/Swift)**, so M5.2 stays open (marked `[~]` in ROADMAP) until a Mac builds it on a simulator — scaffolded, not claimed green.

**Next: M5.2 on-device run + M5.3/M5.4 — all macOS-only.** M5.3 (Keychain `KeyStore`, `os_log` `LogSink`, photo picker, background-refresh sync; APNs push inherits M4.4's server block) and M5.4 (SwiftUI parity screens for M1–M4) require Xcode to compile/validate — do not write large unverifiable Swift blind in this container. The verifiable half (Swift binding generation) is already CI-gated on Linux.

**Wire contract confirmed** (server + reference CLI, M2.1 research): `/sync` returns only `rooms.join`; state/membership events flow inline in `timeline.events` (no separate `state` block); an invite doesn't surface until you're joined; the messages endpoint (`rooms/:id/messages`) takes only `limit` (no `from`/`dir` cursor yet); send txn ids are **not** honored server-side (dedup locally if needed).
