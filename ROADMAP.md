...
**Exit criteria:** install the app, register or log into a real running homeserver, stay logged in across restarts, log out cleanly. End-to-end test against a oneshot homeserver. **Progress:** the core-side flows (register/login/restore) are ✅ **proven against a real oneshot homeserver** by the `e2e/` crate; remaining for M1: the auth UI (M1.4) and logout (M1.5).
