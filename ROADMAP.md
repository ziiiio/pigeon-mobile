...
- [x] **M0.4 — Hello-core Android app.** ✅ A bare Jetpack Compose app (`android/app`, `MainActivity.kt`) installs the M0.7 log sink, then calls `coreVersion()` + `selfTestCrypto()` through the generated UniFFI bindings and renders the results. `assembleDebug` produces a debug APK bundling both ABIs' `.so`. **Remaining for the visual gate: a one-time run on an emulator to confirm it displays on screen** (the amd64 dev container can't host an x86_64 emulator; run on a host emulator).
...
**Exit criteria:** an emulator app displays a value computed in Rust; CI builds both lanes; the build is one command. No protocol code yet. **Progress:** the full pipeline round-trips in the build — Rust core → cargo-ndk `.so` (both ABIs) → UniFFI Kotlin → Compose app compiled into a debug APK (`./gradlew assembleDebug`, one command). The only step left is a visual confirmation on an emulator (build-verified, not yet run on-device); then M0 is closed and M1 opens.
