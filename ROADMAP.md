...
- [ ] **M1.5 — logout & token invalidation.** Clear local session + keystore; call the server logout if it exists. Wire a Sign-out action into `HomeScreen`.

**Exit criteria:** install the app, register or log into a real running homeserver, stay logged in across restarts, log out cleanly. End-to-end test against a oneshot homeserver. **Progress:** the core-side flows (register/login/restore) are ✅ **proven against a real oneshot homeserver** by the `e2e/` crate, and the auth UI (M1.4) is ✅ built and building; remaining for M1: logout (M1.5).
